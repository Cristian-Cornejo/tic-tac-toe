...
const props = defineProps<{ isVsPlayer: boolean }>();

const emit = defineEmits(["update:isVsPlayer"]);

const playerButtonOnClick = () =>
  !props.isVsPlayer && emit("update:isVsPlayer", !props.isVsPlayer);

const botButtonOnClick = () =>
  !!props.isVsPlayer && emit("update:isVsPlayer", !props.isVsPlayer);
</script>
<template>
  <div class="game-mode">
    <p>PLAYER VS.</p>
    <button
      class="game-mode__vs-player-button"
      v-on:click="playerButtonOnClick"
      v-bind:class="{ pressed: props.isVsPlayer }"
    >
      PLAYER
    </button>
    <button
      class="game-mode__vs-bot-button"
      v-on:click="botButtonOnClick"
      v-bind:class="{ pressed: !props.isVsPlayer }"
    >
      BOT
    </button>
